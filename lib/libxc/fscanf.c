...
static int getch(long, long);
static int ungetch(long, long);
...
int fscanf(int dev, const char *format, ...)
{
    ...
    buf.ch_avail = FALSE;
    va_start(ap, format);
    ret = _doscan(format, ap, getch, ungetch, dev, (long)&buf);
    va_end(ap);
    return ret;
}
...
static int getch(long dev, long _ch_buf)
{
...
static int ungetch(long dev, long _chbuf)
{
