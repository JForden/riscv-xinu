...
{
    return (be32_to_cpu((*dataptr) >> 32) << 32 |
            be32_to_cpu((*dataptr) >> 32));
}
...
int parseDtbNodes(struct fdt_header *dtb_addr)
{
    ...
    char *name = NULL;
    uint *struct_base =
        (uint *)((uint)dtb_addr + be32_to_cpu(dtb_addr->off_dt_struct));
    void *string_base =
        (void *)((void *)dtb_addr +
                 be32_to_cpu(dtb_addr->off_dt_strings));
    ...
    while (1)
    {
        ...
        case FDT_PROP:
            ...
            char *pname =
                (char *)(string_base + be32_to_cpu(prop->nameoff));
...
int parseDtb(void)
{
    ...
    parseRsvMap((struct fdt_reserve_entry *)((ulong)dtb_addr +
                                             be32_to_cpu(header->
                                                         off_mem_rsvmap)));
